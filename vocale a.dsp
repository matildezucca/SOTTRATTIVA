...
Ebassf1 = fi.highpass(order, fcut) : fi.lowpass(order, fcut) : *(gain) : meter with{
    order = 128;
    fcut = 600;
f1group(x) = hgroup("[01] f1", x);
gain = f1group(vslider("[01] GAIN", -15, -96, +6, 0.01)) : ba.db2linear : si.smoo;
...
gain = f2group(vslider("[01] GAIN", -22, -96, +6, 0.01)) : ba.db2linear : si.smoo;
...
gain = f3group(vslider("[01] GAIN", -28.4, -96, +6, 0.01)) : ba.db2linear : si.smoo;
...
gain = f4group(vslider("[01] GAIN", -29.3, -96, +6, 0.01)) : ba.db2linear : si.smoo;
